...
npm install
npx prisma db push          # create DB tables (needs DATABASE\_URL set)
...
|Key|Description|
|-|-|
|`DATABASE\_URL`|PostgreSQL URL (Supabase / Neon free tier)|
|`JWT\_SECRET`|Any long random string|
|`RAZORPAY\_KEY\_ID`|From Razorpay dashboard|
|`RAZORPAY\_KEY\_SECRET`|From Razorpay dashboard|
|`AGENT\_SECRET`|Random string shared with PC agent|

### Frontend `.env`

|Key|Description|
|-|-|
|`VITE\_API\_URL`|Backend URL e.g. `https://your-app.railway.app/api`|
...
cd frontend
npm run build
# push to GitHub → import in vercel.com → set VITE\_API\_URL
```
...
2. Opens `yourapp.vercel.app/shop/SHOP\_ID`
...
4. Use test keys first (`rzp\_test\_xxx`)
